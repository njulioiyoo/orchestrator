...
const form = useForm({
    ...
    expires_at: props.tenant.expires_at ? props.tenant.expires_at.slice(0, 16) : '',
    is_active: props.tenant.is_active,
    config: {
        allow_registration: props.tenant.config?.allow_registration || false,
        email_verification: props.tenant.config?.email_verification || false,
        two_factor_auth: props.tenant.config?.two_factor_auth || false
    }
})
